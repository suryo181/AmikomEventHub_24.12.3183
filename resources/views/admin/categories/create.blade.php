    ...
    <div class="max-w-2xl">
        <form action="{{ route('admin.categories.store') }}" method="POST" class="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
            ...
            <div class="mb-6">
                ...
                <input type="number" name="events_count" id="events_count" min="0" class="w-40 px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-600 @error('events_count') border-red-500 @enderror" placeholder="0" value="{{ old('events_count', 0) }}">
                @error('events_count')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
